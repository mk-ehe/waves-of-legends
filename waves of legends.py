...
class character:
    ...
    def basic_attack(self,enemy_damage):
        self.health -= (enemy_damage*1.1)


    def ability(self,enemy_damage):
        self.health -= (enemy_damage*1.4)


    def ult(self,enemy_damage):
        self.health -= (enemy_damage*1.9)


    def on_level_stats_growth(self):
       self.update_basic_health *= 1.43
       self.update_basic_damage *= 1.34
       self.update_basic_mana *= 1.32
...
def fight(name,ability_cost,ult_cost):
    ...
    while True:
        ...
        if enemy_attack == 1:
            ...
            if ability_cost < enemy.mana < ult_cost :
                summoner.ability(enemy.damage)
                enemy.mana = enemy.mana - ability_cost
                print(f'\n{name} has chosen: Ability -{ability_cost} mana')
                sleep(0.5)

            elif enemy.mana > ult_cost:
